...
final class AllNotification extends PowerGridComponent
{
    ...
    public function addColumns(): PowerGridColumns
    {
        return PowerGrid::columns()
            ->addColumn('id')
            ->addColumn('user', function (Notification $model) {
                return $model->user->name;
            })
            ->addColumn('title')

            /** Example of custom column using a closure **/
            ->addColumn('title_lower', fn(Notification $model) => strtolower(e($model->title)))
    ...
    // public function actions(\App\Models\Notification $row): array
    // {
    //     return [
    //         Button::add('edit')
    //             ->slot('Edit: '.$row->id)
    ...
    public function actionRules($row): array
    {
        return [
            ...
            Rule::rows()
                ->when(fn($row) => $row->created_at > now()->subMinutes(30))
